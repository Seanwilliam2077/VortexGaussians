...
## News
- **[2026-08]** **The scene results now run on a real captured reconstruction, and half of them survived the move.** A Postshot-trained 3DGS of a campsite — a parked vehicle on a dry lakebed, 3.0M Gaussians at degree-3 SH from an eight-camera 360° rig, trained by a third party and prepared for the renderer by a documented offline step (seeded subsample to 380k, SH truncated to degree 1, opacity floor 0.35, crop, and a scale calibrated against the real vehicle at 5.31 units/metre) — replaces the synthetic test scene in the teaser and the compositing figure. **Compositing transferred and got stronger:** on a capture there is no opacity at which a surface becomes an occluder, so the depth-aware baseline retains only 1.7% of the flame while ours is correct, and sweeping its alpha threshold does not help (0.15/0.30/0.50 byte-identical; 0.90 collapses to the backdrop composite at 361%). **Cost did not transfer:** the fire alone is 60 FPS, but drawing it inside 380k captured splats costs 140 ms of rasterization (6.5 FPS), and the splat-graph combustion loop — which does run on the capture, charring 44k of the vehicle's own splats — costs ~1.2 s per step at that density. Both are reported rather than folded away. Three demo bugs fell out of the port: the scene-scale slider silently clamped any large reconstruction to a tenth of its size, the scene could only be lifted and never moved sideways (so the fire always landed inside the subject), and the spread adjacency hash was sized from scene extent rather than spread radius, scanning 51,035 candidates per query where 405 suffice.
