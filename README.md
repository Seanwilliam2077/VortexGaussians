...
## News
- **[2026-08]** **The scene results now run on real captured reconstructions, and half of them survived the move.** Three scenes, ordered by how well a reader can check them: the synthetic 45.8k test `.ply`; **`kitchen` from Mip-NeRF 360** in the pre-trained 3DGS release (1.85M Gaussians at degree-3 SH, prepared by a documented offline step to 700k splats **keeping degree 3** — a texture-backed SH path was added for bands 2–3 — with an opacity floor of 0.35 and a crop), which carries the teaser and the compositing figure and which anyone can download and repeat; and a Postshot-trained campsite from a third party (3.0M Gaussians, reduced to 380k at degree 1, scale calibrated against the real vehicle at 5.31 units/metre), which is not ours to redistribute and which carries the combustion loop. **Compositing transferred and got stronger:** on a capture there is no opacity at which a surface becomes an occluder, so the depth-aware baseline keeps only 9.2% of the fire while ours is correct, and sweeping its alpha threshold does not help — at 0.15/0.30 the flame is annihilated entirely, at 0.70/0.90 it jumps to 253%, where the baseline simply *is* the backdrop composite (254%). The metric is a fire-on/fire-off frame difference, not a colour threshold: on a saturated reconstruction a threshold counts the scene's own warm surfaces, which is how we mismeasured this at first. **Cost did not transfer:** paired in one session at 1280×800, the fire alone is 16.9 ms (59 FPS), +45.8k synthetic is 32.3 ms, +380k campsite is 160.2 ms, +700k `kitchen` is 284.0 ms (3.5 FPS); the simulation step is untouched throughout, so the whole deficit is rasterization. Subtracting the fire's own 4.2 ms draw, the marginal cost per scene Gaussian is near-constant and drifts *up* with scene size (324/375/379 ns), correcting an earlier claim that it fell. The splat-graph combustion loop does run on the capture, charring 44k of the vehicle's own splats, at ~1.2 s per step. All of it is reported rather than folded away. Three demo bugs fell out of the port: the scene-scale slider silently clamped any large reconstruction to a tenth of its size, the scene could only be lifted and never moved sideways (so the fire always landed inside the subject), and the spread adjacency hash was sized from scene extent rather than spread radius, scanning 51,035 candidates per query where 405 suffice.
